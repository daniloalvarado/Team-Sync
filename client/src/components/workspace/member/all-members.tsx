  ...
  const handleSelect = (roleId: string, memberId: string) => {
    ...
    mutate(payload, {
      onSuccess: () => {
        ...
        toast({
          title: "Éxito",
          description: "Rol del miembro cambiado exitosamente",
          variant: "success",
        });
      },
      onError: (error) => {
            ...
            <div className="flex items-center gap-3">
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    {translateRole(member.role.name)}{" "}
                ...
                {canChangeMemberRole && (
                  <PopoverContent className="p-0" align="end">
                    <Command>
                      <CommandInput
                        placeholder="Selecciona un nuevo rol..."
                        disabled={isLoading}
                      ...
                      <CommandList>
                        {isLoading ? (
                          ...
                          <>
                            <CommandEmpty>No hay roles encontrados.</CommandEmpty>
                            <CommandGroup>
                              {roles?.map(
                                (role) =>
                                  role.name !== "OWNER" && (
                                    ...
                                    >
                                      ...
                                      <p className="text-sm text-muted-foreground">
                                        {role.name === "ADMIN" &&
                                          `Puede ver, crear, editar tareas, proyecto y gestionar configuración.`}

                                        {role.name === "MEMBER" &&
                                          `Puede ver y editar solo tareas creadas por él.`}
                                      </p>
                                    </CommandItem>
                                  )
                              )}
