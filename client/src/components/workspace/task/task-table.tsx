...
const DataTableFilterToolbar: FC<DataTableFilterToolbarProps> = ({
  ...
  return (
    <div className="flex flex-col w-full space-y-2.5 lg:flex-row lg:items-center lg:space-y-0 lg:space-x-2 mb-2">
      {/* 1. INPUT DE BÚSQUEDA */}
      <Input
        ...
        // Ajuste de altura para móvil y ID para evitar warnings
        className="h-10 w-full lg:w-[250px] lg:h-8"
        name="task-filter-keyword"
        id="task-filter-keyword"
      />

      {/* 2. GRID DE BOTONES (2 Columnas en móvil, Flex en PC) */}
      <div className="grid grid-cols-2 gap-2 w-full lg:flex lg:w-auto lg:items-center">
        ...
        {/* Botón Reiniciar (Solo visible con filtros activos) */}
        {/* En móvil usa 'col-span-2' para ocupar todo el ancho de la última fila */}
        {Object.values(filters).some(
          ...
          <Button
            disabled={isLoading}
            variant="ghost"
            className="h-8 px-2 lg:px-3 col-span-2 lg:col-span-1 border border-dashed lg:border-none"
            onClick={() =>
              setFilters({
                keyword: null,
                status: null,
